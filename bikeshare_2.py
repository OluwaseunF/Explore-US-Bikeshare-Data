...
def city_input():
    ...
    asking the user with the city they want to analyze
    '''
...
def get_time():
    ...
    '''
    period = input('\nYou want to filter the data by month and day of the month, day of the week, or none at all? Type "no" for no period filter.\n')
    period = period.lower()
...
def month_info(m):      # provide user input for month (all, january, february, march, april, may or june)
    if m == 'month':
...
def day_info(d):       # Ask user for a day
    if d == 'day_of_week':
